module.exports = app=>{
    ...
    // 视频排行榜
    router.get('/video/rank', async (req, res)=> {
        const dayRank = await Video.find().limit(6)
        const weekRank = await Video.find().skip(1).limit(6)
        const monthRank = await Video.find().skip(3).limit(6)
        let catesData = [
            {
                name: '日排行',
                videoList: dayRank
